  ...
  export default {
    ...
    methods: {
      getData () {
        ...
        apiServer
          .get('/buku/' + this.$store.state.user.userId)
          .then(response => {
            this.listBuku = response.data
            console.log('buku:', response.data) // For now, logs out the response
      ...
      ubahBuku (item) {
        this.$router.push('/buku-dosen/' + item.idOutput)
      },
      deleteItem (item) {
        ...
        if (item.idPenelitian) {
          ...
          apiServer
            ...
            .then(response => {
              ...
              if (validationResult) {
                axios.all([
                  apiServer.delete('/buku/' + item.idOutput + '/' + this.$store.state.user.name),
