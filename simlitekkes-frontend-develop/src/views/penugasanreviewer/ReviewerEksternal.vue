<template>
  <v-container fluid>
    <material-card
      color="green"
      title="Penugasan Reviewer Eksternal"
    >
      <div>
        ...
        >
          <template v-slot:top>
            <v-toolbar>
              <v-toolbar-title>Daftar Reviewer Eksternal</v-toolbar-title>
            </v-toolbar>
            <v-divider />
            <v-toolbar
              flat
                      ...
                      <v-row>
                        <v-col cols="12">
                          <v-autocomplete
                            ...
                            item-text="nama"
                            item-value="id"
                            placeholder="Harap masukkan nama atau nidn atau institusi minimal 3 karakter"
                            clearable
                      ...
                      <v-row>
                        <v-col cols="10">
                          <v-autocomplete
                            ...
                            item-text="judul"
                            item-value="id"
                            placeholder="Harap masukkan nomor dokumen minimal 3 karakter"
                            clearable
                          >
                            <template
                              slot="selection"
...
<script>
  ...
  export default {
    ...
    computed: {
      formTitle () {
        return this.isEditing ? 'Ubah Reviewer Eksternal' : 'Tambah Reviewer Eksternal'
      },
      tahunUsulanErrors () {
    ...
    methods: {
      ...
      getDaftarTahunUsulan () {
        apiServer
          .get('/tahun-usulan/eks/' + this.idOrganisasi)
          .then(response => {
      ...
      getDaftarPeneliti (keyword) {
        apiServer
          .get('/peneliti-reviewer-eksternal/' + keyword)
          .then(response => {
      ...
      saveReviewer () {
        ...
        this.editedReviewer.idOrganisasi = 1
        ...
        if (this.errors === false) {
          if (this.isEditing) {
            ...
            apiServer
              .put('/reviewer-eksternal', this.editedReviewer)
              .then(response => {
                this.showSnackbar('success', 'Reviewer Eksternal berhasil diubah')
                this.getData(this.tahunIni)
                this.close()
              })
              .catch(error => {
            ...
            apiServer
              .post('/reviewer-eksternal', this.editedReviewer)
              .then(response => {
                this.showSnackbar('success', 'Reviewer Eksternal berhasil ditambah')
                this.getData(this.tahunIni)
                this.getDaftarTahunUsulan()
                this.close()
              })
